...
export default function Login() {
  ...
  //initialized use Mutation
  const login= useMutation(Requests.loginUser,{
  ...
  //Login Schema is Defined here
  const loginSchema = object().shape({
    email: string().required("please enter email"),
    password: string().required("please enter password")
  });

  //OnSubmit Form usin mutate function is defined
  function postOnSubmit(values:{email:string,password:string}){
      login.mutate({
        email:values.email,
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) =>postOnSubmit(values)
  });
      ...
      <div className="border border-gray-100 bg-white shadow-md  p-4 wdth rounded-lg  h-min">
        ...
        <form action="" onSubmit={formik.handleSubmit}>
          <div className="m-2 mb-4">
            <input type="email" name="email" onBlur={formik.handleBlur} onChange={formik.handleChange} value={formik.values.email} className="rounded-lg p-2 border border-gray-200 outline-2 outline-offset-2 outline-sky-500 w-full placeholder:text-gray-500 focus:placeholder:text-gray-400" id="email" placeholder="Enter your userID" />
            {formik.errors.email ? <p className="text-sm text-red-300">{formik.errors.email}</p> : null}
          </div>
          <div className="m-2 mb-4">
            <div className="flex space-between w-full">
              <input type={show ? "text" : "password"} name="password" onBlur={formik.handleBlur} onChange={formik.handleChange} value={formik.values.password} className="w-full rounded-lg p-2 border border-gray-200 outline-2 outline-offset-2 outline-sky-500 placeholder:text-gray-500 focus:placeholder:text-gray-400" id="password" placeholder="Password" />
            ...
            {formik.errors.password ? <p className="text-sm text-red-300">{formik.errors.password}</p> : null}
          </div>
          <div className="m-2 mb-4">
            {!login.isLoading?<button type="submit" className="rounded-lg p-2 bg-sky-500 hover:bg-sky-600 border-sky-700  w-full" style={{ "color": "white" }}>Submit</button>:
            <SubmitSpinner/>}
          </div>
        </form>
