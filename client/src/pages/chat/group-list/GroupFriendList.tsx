...
export default function GroupFriendList() {
    ...
    const postnav = useContext(UseContext);
    const [profile] = useState<users[]>(lst);
    const [first, setfirst] = useState("");
    const filteredNames=profile.filter(e=>e.name.toLowerCase().includes(first.toLowerCase()));
    
    return (
        <div className={`w-1/3 border max-md:w-screen overflow-auto ${!postnav.isSmallScreen?"max-md:visible max-md:block":"max-md:hidden max-md:invisible"} flex flex-col`}>
            <GroupNav />
            <div className="py-2 px-2 flex border border-gray-200">
                <div className='py-2 px-2 rounded-l-2xl bg-gray-200'>
                ...
                <input type="text" className="w-full px-2 py-2 rounded-r-2xl text-sm bg-gray-200 outline-none" value={first}  onChange={e=>setfirst(e.target.value)} placeholder="Search or start new chat" />
            </div>
            <div className=" flex-1 overflow-auto">
                {filteredNames.length?filteredNames.map((e:users)=><div className="px-3 flex items-center hover:bg-gray-200 cursor-pointer" key={e.id} onClick={()=>postnav.callback(e)} >
                    <div>
