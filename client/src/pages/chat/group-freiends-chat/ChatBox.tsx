            ...
            <div className="flex-1 overflow-auto " style={{ "scrollbarWidth": "thin" }}>
                {!isLoading && <div className="py-2 px-3 justify-content-end">
                    {chat.map((e, i) => {
                        if (e.category === 2) {
                            return (
                                <div className="flex justify-center mb-4" key={i}>
                        ...
                        if (!i || e.user?.date?.date !== chat[i - 1].user?.date?.date) {
                            ...
                            return (
                                <div key={i}>
                                    <div className="flex justify-center mb-2 sticky top-2" >
                                    ...
                                    {e.category ? <IncomingMessages message={e.user?.message as string} date={e.user?.date} name={data.type ? e.user?.name : undefined} image={e.user?.image} /> :
                                        <OutgoingMesswges message={e.user?.message as string} date={e.user?.date} image={e.user?.image} />}
                                </div>
                            )
                        }
                        if (e.category === 0) {
                            ...
                            return (
                                ...
                                <OutgoingMesswges message={e.user?.message as string} date={e.user?.date} image={e.user?.image} key={i} />
                            )
                        }

                        return <IncomingMessages message={e.user?.message as string} date={e.user?.date} name={data.type ? e.user?.name : undefined} image={e.user?.image} key={i} />
                    })}
                </div>}
            </div>
